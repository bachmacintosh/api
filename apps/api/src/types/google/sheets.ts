...
type PivotTable = ExactlyOneOf<{
   ...
   *
   * The filters are applied before aggregating data into the pivot table. The map's key is the column offset of the
   * source range that you want to filter, and the value is the criteria for that column.
   *
   * For example, if the source was C10:E15, a key of 0 will have the filter for column C, whereas the key 1 is for
   * column D.
   *
   * @deprecated This field is deprecated in favor of filterSpecs.
   */
   ...
   *
   * Both criteria and filterSpecs are populated in responses. If both fields are specified in an update request, this
  ...
  valueLayout?: Maybe<PivotValueLayout>;
  /** A list of values to include in the pivot table. */
  values?: PivotValue[];
};

/** A single grouping (either row or column) in a pivot table. */
type PivotGroup = ExactlyOneOf<{
  /** The reference to the data source column this grouping is based on. */
  dataSourceColumnReference: DataSourceColumnReference;
  /**
   * The column offset of the source range that this grouping is based on.
   *
   * For example, if the source was C10:E15, a sourceColumnOffset of 0 means this group refers to column C, whereas the
  ...
  /**
   * The labels to use for the row/column groups which can be customized. For example, in the following pivot table, the
   * row label is Region (which could be renamed to State) and the column label is Product (which could be renamed
   * Item). Pivot tables created before December 2017 do not have header labels. If you'd like to add header labels to
   * an existing pivot table, please delete the existing pivot table and then create a new pivot table with same
   * parameters.
   *
   * ```plaintext
   * +--------------+---------+-------+
   ...
   * columns.
   *
   * By default, we minimize repetition of headings by not showing higher level headings where they are the same. For
   * example, even though the third row below corresponds to "Q1 Mar", "Q1" is not shown because it is redundant with
   * previous rows. Setting repeatHeadings to true would cause "Q1" to be repeated for "Feb" and "Mar".
   *
   * ```plaintext
   * +--------------+
   * | Q1     | Jan |
   ...
   * ```
   */
  repeatHeadings?: Maybe<boolean>;
  /** True if the pivot table should include the totals for this grouping. */
  showTotals?: Maybe<boolean>;
...
interface PivotGroupSortValueBucket {
   ...
   *
   * For example, in a pivot table with one row group & two column groups, the row group can list up to two values. The
   * first value corresponds to a value within the first column group, and the second value corresponds to a value in
   * the second column group. If no values are listed, this would indicate that the row should be sorted according to
   * the "Grand Total" over the column groups. If a single value is listed, this would correspond to using the "Total"
   * of that bucket.
   */
  buckets?: ExtendedValue[];
  /** The offset in the PivotTable.values list which the values in this grouping should be sorted by. */
  valuesIndex?: Maybe<number>;
}
 ...
 */
type PivotGroupRule = ExactlyOneOf<{
  /** A DateTimeRule. */
 ...
 * example, a pivot table that aggregates population by state:
 *
 * ```plaintext
 * +-------+-------------------+
...
interface ManualRuleGroup {
  ...
  /**
   * The items in the source data that should be placed into this group. Each item may be a string, number, or boolean.
   * Items may appear in at most one group within a given ManualRule. Items that do not appear in any group will appear
   * on their own.
   */
  items?: ExtendedValue[];
}
...
interface HistogramRule {
  /**
   * The maximum value at which items are placed into buckets of constant size. Values above end are lumped into a
   * single bucket. This field is optional.
   */
  end?: Maybe<number>;
  /** The size of the buckets that are created. Must be positive. */
  interval?: Maybe<number>;
  /**
   * The minimum value at which items are placed into buckets of constant size. Values below start are lumped into a
   * single bucket. This field is optional.
   */
  start?: Maybe<number>;
}
...
interface DateTimeRule {
  ...
  type?: Maybe<DateTimeRuleType>;
}

/**
 * The available types of date-time grouping rules. This documentation assumes the spreadsheet locale is "en-US", though
 * the actual rendering of the dates and times uses the locale of the spreadsheet for some rule types.
 */
...
interface PivotGroupLimit {
   ...
   *
   * Pivot group limits are applied from lower to higher order number. Order numbers are normalized to consecutive
   * integers from 0.
   *
   * For write request, to fully customize the applying orders, all pivot group limits should have this field set with
   * an unique number. Otherwise, the order is determined by the index in the PivotTable.rows list and then the
   * PivotTable.columns list.
   */
  applyOrder?: Maybe<number>;
  /** The count limit. */
  countLimit?: Maybe<number>;
}
...
interface PivotFilterCriteria {
   ...
   * listed there, it is still hidden if it does not meet the condition.)
   *
   * Condition values that refer to ranges in A1-notation are evaluated relative to the pivot table sheet. References
   * are treated absolutely, so are not filled down the pivot table. For example, a condition value of =A1 on "Pivot
   * Table 1" is treated as 'Pivot Table 1'!$A$1.
   *
   * The source data of the pivot table can be referenced by column header name. For example, if the source data has
   * columns named "Revenue" and "Cost" and a condition is applied to the "Revenue" column with type NUMBER_GREATER and
   * value =Cost, then only columns where "Revenue" > "Cost" are included.
   */
  condition?: BooleanCondition;
  /**
   * Whether values are visible by default. If true, the visibleValues are ignored, all values that meet condition (if
   * specified) are shown. If false, values that are both in visibleValues and meet condition are shown.
   */
  visibleByDefault?: Maybe<boolean>;
  /** Values that should be included. Values not listed here are excluded. */
  visibleValues?: Maybe<string[]>;
}

/** The pivot table filter criteria associated with a specific source column offset. */
type PivotFilterSpec = ExactlyOneOf<{
...
type PivotValue = ExactlyOneOf<{
   ...
   * For example, if the source was C10:E15, a sourceColumnOffset of 0 means this value refers to column C, whereas the
   * offset 1 would refer to column D.
   */
  sourceColumnOffset: number;
}> & {
   ...
   * value. For example, if calculatedDisplayType is specified as PERCENT_OF_GRAND_TOTAL, all the pivot values are
   * displayed as the percentage of the grand total. In the Sheets editor, this is referred to as "Show As" in the value
   * section of a pivot table.
   */
  calculatedDisplayType?: Maybe<PivotValueCalculatedDisplayType>;
  name?: Maybe<string>;
  /**
   ...
   */
  summarizeFunction?: Maybe<PivotValueSummarizeFunction>;
};
